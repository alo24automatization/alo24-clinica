  ...
  try {
    ...
    services &&
      services.map(async (service) => {
        const update = await StatsionarService.findByIdAndUpdate(
          service._id,
          service
        );
    ...
    products &&
      products.map(async (product) => {
        const update = await StatsionarProduct.findByIdAndUpdate(
          product._id,
          product
        );
    ...
    if (updateConnector.discount) {
      await StatsionarDiscount.findByIdAndUpdate(
...
module.exports.prepayment = async (req, res) => {
  try {
    ...
    // CreatePayment
    const newpayment = new StatsionarPayment({
      ...payment,
  ...
  try {
    ...
    if (clientborn) {
        ...
        .then((connectors) =>
          connectors.filter(
            (connector) =>
              ...
              new Date(connector.room.endday) >= new Date(beginDay) &&
              new Date(connector.room.endday) <= new Date(endDay)
          )
        );
      ...
      connectors = await StatsionarConnector.find({
        ...
        .populate("discount")
        .then((statsionars) =>
          statsionars.filter((s) => s?.room?.endday === null)
        );
    } else if (type === "todayPayments") {
