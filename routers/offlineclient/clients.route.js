...
/**
 * Retrieves online clients for specific departments within a clinic.
 *
 * @param {String} clinicaId - The ID of the clinic.
 * @param {Array<String>} departments_id - Array of department IDs to fetch online clients for.
 * @returns {Promise<Array<{name: String, data: Array<Object>}>>} - Returns a promise that resolves to an array of department objects, each containing a department name and its online clients.
 * @throws {Error} - Throws an error if the clinic is not found or if any other error occurs during data retrieval.
 */
module.exports.getDepartmentsOnline = async (clinicaId, departments_id) => {
  try {
    // TODO: Fetch clinic information by its ID
    const clinic = await Clinica.findById(clinicaId);
    if (!clinic) {
      // TODO: Handle case where clinic is not found
      throw new Error("Diqqat! Klinika ma'lumotlari topilmadi.");
    }

    // TODO: Retrieve departments based on clinic ID and department IDs provided
    const departments = await Department.find({
      clinica: clinicaId,
      _id: { $in: departments_id }, // TODO: Use $in operator to handle multiple department IDs
    })
      .select("name room probirka letter floor") // TODO: Select only necessary fields for each department
      .populate("doctor", "firstname lastname") // TODO: Populate doctor information associated with each department
      .lean();

    // TODO: Define start and end of the current day to filter clients based on booking date
    const startOfDay = new Date();
    startOfDay.setUTCHours(0, 0, 0, 0);
    const endOfDay = new Date();
    endOfDay.setUTCHours(23, 59, 59, 999);

    // TODO: Fetch clients for each department concurrently using Promise.all
    const clientsData = await Promise.all(
      departments.map(async (department) => {
        // TODO: Find online clients associated with the department within the current day
        const clients = await OnlineClient.find({
          department: department._id,
          brondate: {
            $gte: startOfDay, // TODO: Filter clients based on the start of the day
            $lt: endOfDay, // TODO: Filter clients based on the end of the day
          },
        }).lean();
        // TODO: Return the department name and associated clients
        return { name: department.name, data: clients };
      })
    );

    // TODO: Return the combined data for all departments
    return clientsData;
  } catch (error) {
    console.log(error.message);
    // TODO: Handle any errors that occur during the process
    throw new Error("Serverda xatolik yuz berdi...");
  }
};
